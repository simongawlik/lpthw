...
2. If this else should never run because if doesn't make sense, then you must 
...
3. Never nest if-statements more than two deep and always try to do them one 
   deep.
...
5. Your boolean tests should be simple. If they are complex, move their 
   calculation to variables earlier in your function and use a good name for 
   the variable.
"""

"""
Rules for Loops
1.  Use a while-loop only to loop forever, and that means probably never. This
    only applies to Python; other languages are different.
2.  Use a for-loop for all kinds of looping, especially if there is a fixed or 
...
2.  The best way to debug a program is to use print to print out the values of
